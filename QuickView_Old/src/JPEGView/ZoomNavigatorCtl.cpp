...
CZoomNavigatorCtl::CZoomNavigatorCtl(CMainDlg* pMainDlg, CPanel* pNavigationPanel) {
	m_pMainDlg = pMainDlg;
...
bool CZoomNavigatorCtl::IsVisible() {
	if (!IsActive()) {
		...
		if (bShowZoomNavigator) {
			CRect imageProcRect = CRect(0,0,0,0);
			CRect navBoundRect = CZoomNavigator::GetNavigatorBound(imageProcRect);
...
CRect CZoomNavigatorCtl::PanelRect() {
	if (IsActive()) {
		CRect imageProcRect = CRect(0,0,0,0);
		CRect rect = CZoomNavigator::GetNavigatorRect(m_pMainDlg->GetCurrentImage(), imageProcRect, m_pNavigationPanel->PanelRect());
...
void CZoomNavigatorCtl::DoDragging(int nDeltaX, int nDeltaY) {
	if (m_bDragging) {
		...
		CRect imageProcRect = CRect(0,0,0,0);
		CRect fullRect = CZoomNavigator::GetNavigatorRect(m_pMainDlg->GetCurrentImage(), imageProcRect, m_pNavigationPanel->PanelRect());
...
bool CZoomNavigatorCtl::OnMouseMove(int nOldX, int nOldY) {
	...
	CRect imageProcRect = CRect(0,0,0,0);
	CRect zoomHotArea = CZoomNavigator::GetNavigatorBound(imageProcRect);
...
void CZoomNavigatorCtl::OnPaint(CDC& paintDC, CRectF visRectZoomNavigator, const CImageProcessingParams* pImageProcParams, 
	...
	if (IsActive()) {
		m_zoomNavigator.PaintZoomNavigator(m_pMainDlg->GetCurrentImage(), visRectZoomNavigator, PanelRect(), 
			m_pMainDlg->GetMousePos(), *pImageProcParams, eProcessingFlags, dRotationAngle, pTrapezoid, paintDC);
	}
}