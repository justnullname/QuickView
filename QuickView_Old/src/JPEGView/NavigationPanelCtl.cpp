...
CNavigationPanelCtl::CNavigationPanelCtl(CMainDlg* pMainDlg, bool* pFullScreenMode) : CPanelController(pMainDlg, false) {
	...
	m_pPanel = m_pNavPanel = new CNavigationPanel(pMainDlg->m_hWnd, this, pMainDlg->GetKeyMap(), pFullScreenMode, &(CMainDlg::IsCurrentImageFitToScreen), pMainDlg);
...
bool CNavigationPanelCtl::IsVisible() {
	bool bMouseInNavPanel = m_bMouseInNavPanel;
	return m_bEnabled && !(m_fCurrentBlendingFactorNavPanel <= 0.0f && !bMouseInNavPanel) &&
		/* !m_pMainDlg->IsDoCropping() && */ (m_pMainDlg->IsMouseOn() || bMouseInNavPanel);
}
...
bool CNavigationPanelCtl::OnMouseMove(int nX, int nY) {
	...
	bool bImageProcPanelShown = false; // m_pMainDlg->GetImageProcPanelCtl()->IsVisible();
...
void CNavigationPanelCtl::HideNavPanelTemporary(bool bForce) {
	if (bForce || !m_bMouseInNavPanel /*|| m_pMainDlg->IsCropping()*/) {
		m_bInNavPanelAnimation = true;
