...
namespace {
    ...
    float EstimateFramePeakScRgb(const QuickView::RawImageFrame& frame) {
        if (frame.format != QuickView::PixelFormat::R32G32B32A32_FLOAT || !frame.pixels || frame.width <= 0 || frame.height <= 0) {
            return 1.0f;
        }
    ...
    QuickView::ToneMapSettings BuildToneMapSettings(
        const QuickView::RawImageFrame& frame,
        const QuickView::DisplayColorState& displayState)
    {
        QuickView::ToneMapSettings settings = {};

        const float paperWhiteScRgb = (std::max)(displayState.GetSdrWhiteScale(), 1.0f);
        const float peakNits = (std::max)(displayState.maxLuminanceNits, displayState.sdrWhiteLevelNits);
        ...
        const float highlightCompression =
            sqrtf((std::max)(settings.contentPeakScRgb / (std::max)(headroom, 1.0f), 1.0f));
        float averageCompression = 1.0f;
        if (contentAverageScRgb > 0.0f) {
            const float displayAverageScRgb =
                (std::max)(displayState.maxFullFrameLuminanceNits, displayState.sdrWhiteLevelNits) / 80.0f;
            averageCompression =
                sqrtf((std::max)(contentAverageScRgb / (std::max)(displayAverageScRgb, 1.0f), 1.0f));
        }

        settings.exposure = 1.0f / (std::max)(highlightCompression * averageCompression, 1.0f);
        ...
        if (frame.hdrMetadata.gainMapApplied) {
            ...
            if (appliedHeadroom > 0.0f) {
                ...
                } else if (displayHeadroom > appliedHeadroom + 0.25f && displayState.advancedColorActive) {
                    const float recovery = (std::min)(displayHeadroom - appliedHeadroom, 1.5f);
                    settings.exposure *= (1.0f + recovery * 0.08f);
                }
            }
        } else if (frame.hdrMetadata.hasGainMap && frame.hdrMetadata.gainMapAlternateHeadroom > 0.0f) {
            const float displayHeadroom = displayState.GetHdrHeadroomStops();
            if (displayHeadroom + 0.1f < frame.hdrMetadata.gainMapAlternateHeadroom) {
                const float mismatch = frame.hdrMetadata.gainMapAlternateHeadroom - displayHeadroom;
...
default:
    ...
    if (frame.format == QuickView::PixelFormat::R32G32B32A32_FLOAT && !m_isAdvancedColor) {
        ...
        const float sceneScale = toneMapSettings.exposure * toneMapSettings.paperWhiteScRgb /
            sqrtf((std::max)(toneMapSettings.contentPeakScRgb / (std::max)(toneMapSettings.displayPeakScRgb, 1.0f), 1.0f));
        for (int y = 0; y < frame.height; ++y) {
            const float* srcRow = reinterpret_cast<const float*>(frame.pixels + static_cast<size_t>(y) * frame.stride);
