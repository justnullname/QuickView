...
CRect CNavigationPanel::PanelRect() {
	if (m_nWidth == 0) {
		...
		for (iter = m_controls.begin( ); iter != m_controls.end( ); iter++ ) {
			...
			if (pButton != NULL) {
				m_nOptimalWidth += m_nHeight - m_nBorder + m_nGap;
			} else {
				CGapCtrl* pGap = dynamic_cast<CGapCtrl*>(iter->second);
				if (pGap != NULL) {
					m_nOptimalWidth += (int)(pGap->GapWidth()*m_fDPIScale*m_fAdditionalScale);
				}
			}
		}
		m_nOptimalWidth += m_nBorder*2 - m_nGap;
		m_nWidth = m_nOptimalWidth;
	}
...
bool CNavigationPanel::AdjustMaximalWidth(int nMaxWidth) {
	int nUsedWidth = m_nOptimalWidth;
...
void CNavigationPanel::PaintZoomFitToggleBtn(void* pContext, const CRect& rect, CDC& dc) {
	...
	if (pNavPanel == nullptr) return; // Safety check
...
LPCTSTR CNavigationPanel::WindowModeTooltip(void* pContext) {
	CNavigationPanel* pNavPanel = (CNavigationPanel*)pContext;
	if (pNavPanel == nullptr) return _T(""); // Safety check
...
CString CNavigationPanel::GetTooltip(CKeyMap* keyMap, LPCTSTR tooltip, int command) {
	CString sTooltip(tooltip);
	CString sShortcut = keyMap->GetKeyStringForCommand(command);
	if (!sShortcut.IsEmpty()) {
		sTooltip += _T(" (") + sShortcut + _T(")");
	}
	return sTooltip;
}
CNavigationPanel::CNavigationPanel(HWND hWnd, INotifiyMouseCapture* pNotifyMouseCapture, CKeyMap* keyMap, bool* pFullScreenMode,
	...
	AddUserPaintButton(ID_btnHome, (LPCTSTR)NULL, &PaintHomeBtn);
	AddUserPaintButton(ID_btnPrev, (LPCTSTR)NULL, &PaintPrevBtn);
	AddGap(ID_gap1, 8);
	AddUserPaintButton(ID_btnNext, (LPCTSTR)NULL, &PaintNextBtn);
	AddUserPaintButton(ID_btnEnd, (LPCTSTR)NULL, &PaintEndBtn);
	AddGap(ID_gap2, 16);
	if (CSettingsProvider::This().AllowFileDeletion()) {
		AddUserPaintButton(ID_btnDelete, (LPCTSTR)NULL, &PaintDeleteBtn);
		AddGap(ID_gap3, 16);
	}
	AddUserPaintButton(ID_btnZoomMode, (LPCTSTR)NULL, &PaintZoomModeBtn);
	AddUserPaintButton(ID_btnFitToScreen, (LPCTSTR)NULL, &PaintZoomFitToggleBtn);
	AddUserPaintButton(ID_btnWindowMode, (LPCTSTR)NULL, &PaintWindowModeBtn);
	AddGap(ID_gap4, 16);
	AddUserPaintButton(ID_btnRotateCW, (LPCTSTR)NULL, &PaintRotateCWBtn);
	AddUserPaintButton(ID_btnRotateCCW, (LPCTSTR)NULL, &PaintRotateCCWBtn);
	AddUserPaintButton(ID_btnRotateFree, (LPCTSTR)NULL, &PaintFreeRotBtn);
	AddUserPaintButton(ID_btnPerspectiveCorrection, (LPCTSTR)NULL, &PaintPerspectiveBtn);
	AddGap(ID_gap5, 16);
	AddUserPaintButton(ID_btnKeepParams, (LPCTSTR)NULL, &PaintKeepParamsBtn);
	AddUserPaintButton(ID_btnLandscapeMode, (LPCTSTR)NULL, &PaintLandscapeModeBtn);
	AddGap(ID_gap6, 16);
	AddUserPaintButton(ID_btnShowInfo, (LPCTSTR)NULL, &PaintInfoBtn);

	// Tooltips  
	GetBtnHome()->SetTooltip(GetTooltip(m_keyMap, _T("First image"), 0));
	GetBtnPrev()->SetTooltip(GetTooltip(m_keyMap, _T("Previous image"), IDM_PREV));
	GetBtnNext()->SetTooltip(GetTooltip(m_keyMap, _T("Next image"), IDM_NEXT));
	GetBtnEnd()->SetTooltip(GetTooltip(m_keyMap, _T("Last image"), 0));
	if (GetBtnDelete() != NULL) {
		GetBtnDelete()->SetTooltip(GetTooltip(m_keyMap, _T("Delete image"), IDM_MOVE_TO_RECYCLE_BIN));
	}
	GetBtnZoomMode()->SetTooltip(GetTooltip(m_keyMap, _T("Zoom mode"), IDM_ZOOM_MODE));
	GetBtnFitToScreen()->SetTooltipHandler(&ZoomFitToggleTooltip, this);
	GetBtnWindowMode()->SetTooltipHandler(&WindowModeTooltip, this);
	GetBtnRotateCW()->SetTooltip(GetTooltip(m_keyMap, _T("Rotate 90") + CString(_T("\x00B0")), IDM_ROTATE_90));
	GetBtnRotateCCW()->SetTooltip(GetTooltip(m_keyMap, _T("Rotate 270") + CString(_T("\x00B0")), IDM_ROTATE_270));
	GetBtnRotateFree()->SetTooltip(GetTooltip(m_keyMap, _T("Rotate freely"), IDM_ROTATE));
	GetBtnPerspectiveCorrection()->SetTooltip(GetTooltip(m_keyMap, _T("Perspective correction"), IDM_PERSPECTIVE));
	GetBtnKeepParams()->SetTooltip(GetTooltip(m_keyMap, _T("Keep parameters"), IDM_KEEP_PARAMETERS));
	GetBtnLandscapeMode()->SetTooltip(GetTooltip(m_keyMap, _T("Landscape mode"), IDM_LANDSCAPE_MODE));
	GetBtnShowInfo()->SetTooltip(GetTooltip(m_keyMap, _T("Show file info"), IDM_SHOW_FILEINFO));
}
