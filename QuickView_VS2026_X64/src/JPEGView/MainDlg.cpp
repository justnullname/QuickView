...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {	
	...
	m_pEXIFDisplayCtl = new CEXIFDisplayCtl(this, NULL); // m_pImageProcPanelCtl->GetPanel());
	m_pPanelMgr->AddPanelController(m_pEXIFDisplayCtl);

	// Create unsharp mask panel
	m_pUnsharpMaskPanelCtl = new CUnsharpMaskPanelCtl(this, NULL); // m_pImageProcPanelCtl->GetPanel());
	m_pPanelMgr->AddPanelController(m_pUnsharpMaskPanelCtl);

	// Create rotation panel
	m_pRotationPanelCtl = new CRotationPanelCtl(this, NULL); // m_pImageProcPanelCtl->GetPanel());
	m_pPanelMgr->AddPanelController(m_pRotationPanelCtl);

	// Create perspective correction panel
	m_pTiltCorrectionPanelCtl = new CTiltCorrectionPanelCtl(this, NULL); // m_pImageProcPanelCtl->GetPanel());
	m_pPanelMgr->AddPanelController(m_pTiltCorrectionPanelCtl);

	// Create navigation panel
	m_pNavPanelCtl = new CNavigationPanelCtl(this, &m_bFullScreenMode); // m_pImageProcPanelCtl->GetPanel(), &m_bFullScreenMode);
	m_pPanelMgr->AddPanelController(m_pNavPanelCtl);

	// Create window button panel (on top, right)
	m_pWndButtonPanelCtl = new CWndButtonPanelCtl(this, NULL); // m_pImageProcPanelCtl->GetPanel());
...
LRESULT CMainDlg::OnSize(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
	...
	if (nWidth > 0 && nHeight > 0) {
		m_clientRect = CRect(0, 0, nWidth, nHeight);
		if (m_pPanelMgr != NULL) {
			// m_pPanelMgr->OnResize(m_clientRect);
		}
...
void CMainDlg::PaintToDC(CDC& dc, const CRect& paintRect) {
	...
	CTrapezoid trapezoid = m_pTiltCorrectionPanelCtl->IsVisible() ? m_pTiltCorrectionPanelCtl->GetCurrentTrapezoid(CZoomNavigator::GetNavigatorRect(m_pCurrentImage, CRect(0,0,0,0), m_pNavPanelCtl->PanelRect()).Size()) : CTrapezoid(); // m_pImageProcPanelCtl->PanelRect()
...
LRESULT CMainDlg::OnLButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	this->SetCapture();
	bool isCropping = false; // m_pCropCtl->IsCropping();
...
LRESULT CMainDlg::OnLButtonUp(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	if (m_bZoomMode) {
		...
		EndDragging();
	} else if (false) { // m_pCropCtl->IsCropping()) {
...
LRESULT CMainDlg::OnLButtonDblClk(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	if (!m_bDragging && true) { // !m_pCropCtl->IsCropping()) {
		if (m_pPanelMgr->OnMouseLButton(MouseEvent_BtnDblClk, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))) {
			return 0;
		}
	}
	if (true && m_pCurrentImage != NULL) { // !m_pCropCtl->IsCropping() && m_pCurrentImage != NULL) {
		// Smart Fullscreen Exit: Double click on background exits fullscreen
		if (m_bFullScreenMode) {
			CPoint pt(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			
			// Calculate image rectangle manually using offsets and virtual size
			// m_offsets is center based, need to convert to top-left
			int nX = m_clientRect.Width() / 2 + m_offsets.x - m_virtualImageSize.cx / 2;
			int nY = m_clientRect.Height() / 2 + m_offsets.y - m_virtualImageSize.cy / 2;
			CRect imageRect(nX, nY, nX + m_virtualImageSize.cx, nY + m_virtualImageSize.cy);

			if (!imageRect.PtInRect(pt)) {
				// Clicked outside image -> Exit Fullscreen
		...
		// Double-click toggle logic based on m_dZoom state:
		// m_dZoom < 0: Auto/Fit mode
		// m_dZoom > 0: User-set zoom level
		
		if (m_dZoom < 0) {
			// Currently in Auto/Fit mode -> go to 100%
			m_dZoom = 1.0;
			m_bUserZoom = true;
		...
		} else if (fabs(m_dZoom - 1.0) < 0.01) {
			// Currently at ~100% -> go to Fit (use existing command that works)
			ExecuteCommand(IDM_FIT_TO_SCREEN);
		} else {
			// Currently zoomed in or out -> go to 100%
			m_dZoom = 1.0;
			m_bUserZoom = true;
			m_isUserFitToScreen = false;
			m_offsets = CPoint(0, 0);
...
LRESULT CMainDlg::OnContextMenu(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	...
	if (false) { // m_pCropCtl->IsCropping()) {
...
void CMainDlg::ExecuteCommand(int nCommand) {
	...
	switch (nCommand) {
		...
		case IDM_SAVE_PARAM_DB:
			if (m_pCurrentImage != NULL && true && !m_bKeepParams) { // !m_bMovieMode
		...
		case IDM_COPY_SEL:
			if (m_pCurrentImage != NULL) {
				CClipboard::CopyFullImageToClipboard(this->m_hWnd, m_pCurrentImage, *m_pImageProcParams, 
					...
					CRect(0,0,0,0), NULL); // m_pCropCtl->GetImageCropRect(false), NULL);
				this->Invalidate(FALSE);
			}
			break;
...
void CMainDlg::GotoImage(EImagePosition ePos, int nFlags) {
	...
	if (bCheckIfSameImage && (m_pFileList == pOldFileList && (nOldFrameIndex == nFrameIndex || bNoWrapAroundEdgeFrame) && !m_pFileList->ChangedSinceCheckpoint())) {
		if (false && m_bAutoExit) { // m_bMovieMode
			CleanupAndTerminate();
		} else {
...
void CMainDlg::ZoomToSelection() {
	CRect zoomRect(0,0,0,0); // m_pCropCtl->GetImageCropRect(false));
...
// StartMovieMode removed

// StopMovieMode removed
...
void CMainDlg::MouseOff() {
	if (m_bMouseOn) {
		if (m_nMouseY < m_clientRect.bottom - 0 && // m_pImageProcPanelCtl->PanelRect().Height() && 
...
CRect CMainDlg::GetZoomTextRect(CRect imageProcessingArea) {
	...
	if (false) { // m_pImageProcPanelCtl->IsVisible()) {
...
void CMainDlg::DisplayFileName(const CRect& imageRect, CDC& dc, double dZoom) {
	if (!m_bShowFileName && !m_bShowZoomFactor) return;

	CString sFileName;
	if (m_bShowFileName) {
		sFileName = CurrentFileName(true);
	}

	/*
	if (m_bShowZoomFactor) {
		CString sZoom;
		sZoom.Format(_T(" (%d%%)"), (int)(dZoom * 100 + 0.5));
		sFileName += sZoom;
	}
	*/
	...
	if (!sFileName.IsEmpty()) {
		...
		HelpersGUI::SelectDefaultFileNameFont(dc);
		HelpersGUI::DrawTextBordered(dc, sFileName, imageRect, DT_LEFT | DT_TOP);
	}
}

void CMainDlg::DisplayErrors(CJPEGImage* pImage, const CRect& clientRect, CDC& dc) {
	if (pImage == NULL) return;
	if (m_nLastLoadError != HelpersGUI::FileLoad_Ok) {
		HelpersGUI::DrawImageLoadErrorText(dc, clientRect, CurrentFileName(true), m_nLastLoadError, 0);
	}
}
...
LRESULT CMainDlg::OnClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled) {
	...
	if (!m_bFullScreenMode) {
		GetWindowPlacement(&m_storedWindowPlacement);
...
LRESULT CMainDlg::OnGetMinMaxInfo(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	...
	pMMI->ptMinTrackSize.x = MIN_WND_WIDTH;
